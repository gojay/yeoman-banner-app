...
define(['angular', 'raphael', 'raphael-filter', 'raphael-transform', 'raphael-json', 'raphael-group'], function (angular) {
  ...
  angular.module('bannerAppApp.controllers.RaphaelCtrl', [])
    .controller('RaphaelCtrl', ['$scope', '$compile', '$timeout', 'Banner', 
    	function ($scope, $compile, $timeout, Banner) {
    	window.$scope = $scope;
		...
		).attr({fill: 'white'}).shadow(0, 1, 4, 0.6).click(onClickHandler);
		...
		function onClickHandler(e){
			...
			angular.forEach(fts, function(ft){ ft.hideHandles(); });
    	...
    	$scope.$watch('draw', function(value){
    		...
    		elFts[$scope.banner.selected].setOpts({ draw: [ value == 'bbox' ? 'bbox' : null, value == 'circle' ? 'circle' : null ] });
    	});
